(function () {
  function buildToc() {
    ...
    const hiddenContent = article.closest('.content-hidden');
    if (hiddenContent) {
      const observer = new MutationObserver(() => {
        if (!hiddenContent.classList.contains('content-hidden')) {
          observer.disconnect();
          buildToc();
        }
      });
      observer.observe(hiddenContent, { attributes: true, attributeFilter: ['class'] });
      return;
    }
